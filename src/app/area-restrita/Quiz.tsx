...
import { QuizData, Questao } from "../types/types";

export default function Quiz({ quizTitle }: { quizTitle: string }) {
  const quizData: QuizData | undefined = getQuizData(quizTitle);
  ...
  const [showPainel, setShowPainel] = useState<boolean>(false);
  const [loadingPainel, setLoadingPainel] = useState<boolean>(false);
  const [respostaPainel, setRespostaPainel] = useState<string>("");
  ...
  const questao: Questao = quizData.data[questaoAtual];
